...
ArrayList* al_clone(ArrayList* this) //probada
{
    ...
    if(this!=NULL)
    {
        ...
        while(this->reservedSize > returnAux->reservedSize)
        {
            ...
            for(i=0;i<this->size;i++)
            {
                (returnAux+i)->pElements=*(this->pElements+i);
            }
...
int al_push(ArrayList* this, int index, void* pElement) //probada
{
    ...
    if(this!=NULL && pElement!=NULL  && index >= 0  && index <= this->size ) // OJO ACA; creo q el index debe ser menor a size, no menor e igual
    {
        if(index==this->size) // OJO ACA; creo q debe ser reservedSIze en vez de index
        {
...
void* al_pop(ArrayList* this,int index) //probada
{
    ...
    if(this!=NULL && index<this->size && index>=0 ) // ojo , ak index < size
    {
...
int contract(ArrayList* this,int index)
{
    ...
    if(this!=NULL && index >= 0  && index < this->size)
    {
        ...
        {
            *(this->pElements+i)=*(this->pElements+(i+1)); // OJO ACA; puede estar un error en el +1
        }
